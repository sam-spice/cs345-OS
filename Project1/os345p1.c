...
int P1_shellTask(int argc, char* argv[])
{
	...
	while (1)
	{
		...
		// ?? must use malloc for argv storage!
		{
			...
			//shift to lower case unless 
			bool in_para = FALSE;
			for (int j = 0; sp[j]; j++) {
				if (sp[j] == '"'){
					in_para ^= TRUE;
				}
				if (!in_para) sp[j] = tolower(sp[j]);
			}
			// parse input string
			while ((sp = strchr(sp, ' ')))
			{ 
			...
			// malloc new argv!
			char * tempBuff[MAX_ARGS];
			//for(i = 0; i < newArgc)
			newArgv = (char **)malloc(sizeof(char*) * argc);
			for (int i = 0; i < newArgc; i++) {
				newArgv[i] = (char *)malloc(sizeof(char *) * (strlen(myArgv[i]) + 1));
				newArgv[i] = strcpy(newArgv[i], myArgv[i]);
			}
			char * final_entry = newArgv[newArgc - 1];
			if (final_entry[0] == 0) {
				printf("in this loop");
				newArgc--;
			}
		...
		for (found = i = 0; i < NUM_COMMANDS; i++)
		{
			...
			{
				if (!background) {// command found, make implicit call thru function pointer
		...
		if (!found)	printf("\nInvalid command!");
		if (background) printf("background!\n\n");
